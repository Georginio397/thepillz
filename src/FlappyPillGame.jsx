...
function FlappyPillGame({ onRequireLogin }) {
  ...
  const [assetsReady, setAssetsReady] = useState(false); // ✅ nou

  // imagini globale (sprite-uri)
  const assets = useRef({});
  ...
  // ✅ funcție pentru încărcare + decodare imagini
  const loadImage = (src) => {
    return new Promise((resolve, reject) => {
      ...
      img.onload = async () => {
        try {
          // forțăm decoding înainte să începem
  ...
  // ✅ preloading tuturor asset-urilor
  useEffect(() => {
    const preload = async () => {
      const sources = {
        bg1: "/sprites/bg1.png",
      ...
      setAssetsReady(true); // ✅ toate sunt gata
    };

    preload();
  }, []);

  // audio context (preload sunet)
  ...
  const start = () => {
    if (!assetsReady) return; // ✅ nu începe până nu e gata
...
{!assetsReady && (
      ...
      {gameOver && <button onClick={start}>Restart</button>}
